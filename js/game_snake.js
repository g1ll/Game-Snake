//EXEMPLO UTILIZANDO APENAS FUNÇÕES NATIVAS E SEM O RANKING COM CHAMADA AJAX
...
function canvasApp() {
    ...
    document.getElementById("canvas").addEventListener("touchstart", function (event) {
        if (pausedgame)
            init();//Inicia o jogo com um toque na tela
...
//        ctx.strokeRect(0, 0, w, h);
        ...
        var text3 = "Ou toque na tela touch na direção desejada!";
    ...
    function gameOver() {
        ...
        // o valor xText será sempre igual à metade do canvas menos a metada da largura do texto
        var xText = (canvas.width / 2) - (textWidth / 2);
        var yText = 7*text_size+10; //Posiciona o texto na altura 14 pixel, esta configuração depende do tamanho da fonte        
        ctx.fillText(text1, xText, yText); //Desenha o texto na tela
    }
...
//        getLastRanking(); //Armazenando a pontuação do último Ranking
        ...
        score = 0;//Reiniciando a pontuação
        //Configurações de tamanho de tela
        size = getWindowSize();
        w = size.w * 0.99;
        h = size.h * 0.99;
        cw = (size.w <= size.h ? cw = size.w * 0.05 : size.h * 0.05);//Definindo o tamanho padrão das células (Tiles) do mapa do jogo
